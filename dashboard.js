...
async function fetchFromLocalFirat(query) {
  return new Promise((resolve) => {
    chrome.storage.local.get(['firatBoruList'], (res) => {
      ...
      // Türkçe karakterlere duyarlı temizleme işlemi uyguluyoruz
      ...
      for (const item of matches) {
        ...
        // Jenerik ürün adı üzerinden images klasöründeki görsel adını oluşturuyoruz
...
// Fırat Boru Görsel Eşleştirme Algoritması ve Mevcut Görsel Listesi
const FIRAT_IMAGES = [
...
function getFiratProductImage(code) {
  ...
  // Karakter sayısını sondan geriye doğru azaltarak benzer bir görsel var mı diye ararız
...
document.addEventListener('error', (e) => {
  if (e.target.tagName === 'IMG' && e.target.classList.contains('product-img')) {
    ...
    // Dosya adında harf varsa isim bazlı jenerik görseldir (Örn: pvc_cift_catal.jpeg)
    const isNameBased = /[a-zA-Z]/.test(filename);
    
    // a. Eğer isim bazlı görsel yüklenemediyse, kodlu görseli dene
    ...
    // b. Eğer kodlu görsel de yüklenemediyse, uzantıları ve son çare logo.png'yi dene
